...
Grid2Options.indicatorTypesOrder= { tooltip = 1, alpha = 2, background = 3, border = 4, glowborder = 5, multibar = 6, bar = 7, text = 8, square = 9, shape = 10, icon = 11, icons = 12, privateauras = 13, privateaurasdispells = 14, portrait = 15 }
...
do
	...
	local function NewIndicator()
		...
		if (newIndicatorName and newIndicatorName ~= "") then
			...
			elseif (newIndicatorValues.type == "privateauras") then
				dbx.level = 8
				dbx.maxIcons = 2
			elseif	(newIndicatorValues.type == "privateaurasdispells") then
				dbx.level = 7
			end
	...
	local function RenameIndicatorReal(old_name, new_name)
		...
		if old_sideKick then
			db.indicators[new_indicator.sideKick.name] = db.indicators[old_sideKick.name]
			db.indicators[old_sideKick.name] = nil
