...
local function TestMode(info)
	...
	if info.handler[1] then
		...
		layoutName = layouts[maxPlayers]
			or layouts[(maxPlayers == 1 and "solo") or (maxPlayers == 5 and "party") or "raid"]
	else
		maxPlayers = strfind(info.arg, "raid") and 40 or 5
		layoutName = layouts[info.arg] or layouts["raid"]
	end
	local enabled = not Grid2.testMaxPlayers
		or (
			theme.index ~= Grid2.testThemeIndex
			or layoutName ~= Grid2Layout.testLayoutName
			or maxPlayers ~= Grid2.testMaxPlayers
		)
...
do
	...
	local options_item = {
		...
		frameWidth = {
			...
			step = 1,
			get = function(info)
				return theme.frame.frameWidths[info.handler[1]] or 0
			end,
			set = function(info, v)
				new_sizes[info.handler[1]] = v
		...
		frameHeight = {
			...
			step = 1,
			get = function(info)
				return theme.frame.frameHeights[info.handler[1]] or 0
			end,
			set = function(info, v)
				new_sizes[info.handler[1]] = v
	...
	local function IsHidden(info)
		...
		return not (
			theme.layout.layouts[size]
			or theme.frame.frameWidths[size]
			or theme.frame.frameHeights[size]
			or new_sizes[size]
		)
	end
	...
	MakeFrameSizesOptions = function()
		return options
	end
end
...
local generalOptions = {
	...
	createdesc = {
		order = 199,
		type = "description",
		name = "\n" .. L["Filter Pets by Summoner Class or Pet Names"],
	},
	...
	create = {
		...
		multiline = 9,
		name = L["Pet Filter"],
		desc = L["Comma seperated list of uppercase class names (WARLOCK) and/or pet names (First Letter Capitalized: Flatoom)"],
		get = function()
			return theme.layout.petFilter or ""
		end,
		set = function(info, v)
			theme.layout.petFilter = v or ""
			Grid2Layout:RefreshLayout()
		end,
	},
}
