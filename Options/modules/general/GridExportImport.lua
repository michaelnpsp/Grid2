...
-- Export profile data to lua code, used to generate Grid2/Grid2Defaults.lua and Grid2Options/profiles/*.lua
do
	local function fmtNumber(n)
		local d = n % 1
		if strlen(tostring(d))<=5 then
			return tostring(n)
		else
	...
	local function fmtKey(k)
		if type(k)=='number' then
			return '['..k..']'
		elseif string.match (k,"^[%a_]+$") or string.match (k,"^[%a_]+%d+$") then
			return k
		else
			return '["'..k..'"]'
		end
	...
	local function Serialize(t, noRoot)
		if type(t) == 'boolean' then
			return tostring(t)
		elseif type(t) =='number'then
			return fmtNumber(t)
		elseif type(t) == 'string' then
			return '"' .. C_StringUtil.EscapeQuotedCodes(t) .. '"'
		elseif type(t) == 'table' then
			local r = {}
			for k,v in pairs(t) do
				table.insert( r, string.format( "%s=%s", fmtKey(k), Serialize(v,true) ) )
			end
			return string.format( noRoot and "{%s}" or "{ %s }", table.concat(r, noRoot and "," or ", ") )
		else
			return "nil"
		end
	end

	local function ExportNamespace(namespace, profileName)
		local pf, r = Grid2DB.namespaces[namespace].profiles[profileName]
		if pf then
			r = string.format( '-- %s\nlocal pf = Grid2.db:GetNamespace("%s").profile\n', namespace, namespace )
			for k,v in pairs(pf) do
				r = r .. string.format( 'pf.%s = %s\n', k, Serialize(v) )
			end
		end
		return r or ''
	end
	...
	local function ExportProfile(profileName)
		local db = Grid2DB.profiles[profileName]
		if not db then return end
		local e = {}
		local r = "-- profile\nlocal db = Grid2.db.profile\nlocal s, i, m = db.statuses, db.indicators, db.statusMap\n"
		local function ExportIndicator(k)
			local v = db.indicators[k]
			if not v then return end
			r = r .. string.format( 'i["%s"] = %s\n', k, Serialize(v) )
			local m = db.statusMap[k]
			if m and next(m) then
				r = r .. string.format( 'm["%s"] = %s\n', k, Serialize(m) )
			end
			e[k] = true
		end
		-- metadata
		if db.__template then
			r = string.format( "%s\ndb.__template = '%s'\n", r, db.__template )
		end
		-- statuses
		r = r .. "\n"
	...
	local function RegisterProfile(profileName)
		local r
		local db = Grid2DB.profiles[profileName]
	...
	function Grid2Options:ExportProfileToLua(profileName)
		local r = "-- Autogenerated: " .. date("%Y/%m/%d %H:%M:%S") .. "\n"
		...
		Grid2Options:ExportText('Export', r)
	end
end
