...
-- formatType = "DecimalFormat" | "DurationStackFormat" | "minutesDecimalFormat" | "hoursDecimalFormat"
local function GetFormat(formatType)
	local dbx = Grid2.db.profile.formatting
	local mask = dbx[ "long".. formatType ] or dbx[formatType]
...
local function GetTenths(formatType)
	local dbx   = Grid2.db.profile.formatting
	local short = dbx[ formatType ]
	if short then  -- minutesDecimalFormat" | "hoursDecimalFormat"
		return short:find("%%.1f") and 2 or 1
	else
...
local SetFormat, SetFormatShort
do
	local function IsValidFormat(str)
		if str and str~='' then
			return pcall(string.format, str, 1, 1)
		end
	end
	...
	local function ToDbFormat(formatType, mask, tenths)
		...
		else -- minutesDecimalFormat" | "hoursDecimalFormat"
			return (mask:gsub("%%d",(tenths==1) and "%%.0f" or "%%.1f"))
		end
	end
	function SetFormat(formatType, mask, tenths)
		...
		if long then
			...
			if IsValidFormat(short) and IsValidFormat(long) then
				dbx["short"..formatType] = short
				dbx["long" ..formatType] = long
				if inverted ~= nil then dbx.invertDurationStack = inverted end
			end
		elseif short and IsValidFormat(short) then
			dbx[formatType] = short
		end
		Grid2:UpdateGeneralTextFormatting()
		Grid2Options:UpdateIndicators('text')
		Grid2Options:UpdateIndicators('textaura')
	end
end
...
Grid2Options:AddGeneralOptions( "General", "Text Formatting", {
	...
	dFormatMinutes = {
		type = "input",
		order = 2.1,
		name = L["Remaining time Minutes"],
		desc = L["Examples:\n(%d)\n%d minutes"],
		get = function() return GetFormat("minutesDecimalFormat") end,
		set = function(_,v) SetFormat("minutesDecimalFormat", v, nil) end,
	},
	dFormatMinutesTenths = {
		type = "select",
		order = 2.2,
		name = L["Display tenths of a minute"],
		desc = L["Display tenths of a minute"],
		get = function () return GetTenths("minutesDecimalFormat") end,
		set = function (_, v) SetFormat("minutesDecimalFormat", nil, v) end,
		values = { L["Never"], L["Always"]},
	},
	dFormatHours = {
		type = "input",
		order = 2.3,
		name = L["Remaining time Hours"],
		desc = L["Examples:\n(%d)\n%d hours"],
		get = function() return GetFormat("hoursDecimalFormat") end,
		set = function(_,v) SetFormat("hoursDecimalFormat", v, nil) end,
	},
	dFormatHoursTenths = {
		type = "select",
		order = 2.4,
		name = L["Display tenths of a hour"],
		desc = L["Display tenths of a hour"],
		get = function () return GetTenths("hoursDecimalFormat") end,
		set = function (_, v) SetFormat("hoursDecimalFormat", nil, v) end,
		values = { L["Never"], L["Always"] },
	},
