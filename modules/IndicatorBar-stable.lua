...
local function Bar_Layout(self, parent)
	...
	local width  = self.width  or parent[self.anchorTo]:GetWidth()
	local height = self.height or parent[self.anchorTo]:GetHeight()
	...
	Bar:SetPoint(self.anchor, parent[self.anchorTo], self.anchorRel, self.offsetx, self.offsety)
...
local function Bar_UpdateDB(self, dbx)
	...
	self.anchorTo       = dbx.anchorTo or "container"
	self.anchor         = l.point
...
local function BarColor_SetBarColorInverted(self, parent, r, g, b, a)
	...
	if not self.dbx.parentBar then
		parent[self.anchorTo]:SetVertexColor(r, g, b, a)
	end	
end
